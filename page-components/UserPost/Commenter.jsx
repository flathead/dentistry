  ...
  const onSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        toast.success('Вы добавили комментарий');
    ...
    <form onSubmit={onSubmit}>
      <Container className={styles.poster}>
        ...
        <Input
          ...
          ariaLabel='Добавьте комментарий'
        />
  ...
  return (
    <div className={styles.root}>
      <h3 className={styles.heading}>
        Ответить{' '}
        <TextLink href={`/user/${post.creator.username}`} passHref color='link'>
          @{post.creator.username}
        </TextLink>
      </h3>
      {loading ? (
        <LoadingDots>Загрузка</LoadingDots>
      ) : data?.user ? (
