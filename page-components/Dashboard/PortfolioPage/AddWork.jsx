  ...
  const onSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        if (res.status === 200) {
          toast.success('Вы успешно добавили работу!');
        }
    ...
    <Card className={styles.addWrapper}>
      <div className={styles.buttonContainer}>
        <NextUIButton
          ...
          onClick={formHandle}
        >
          {visibility ? 'Скрыть' : 'Добавить работу'}
        </NextUIButton>
        <p>
          {portfolios && portfolios.length >= 1
            ? `${createLabel(portfolios.length, [
                'Добавлена',
                'Добавлено',
                'Добавлено',
              ])} ${portfolios.length} ${createLabel(portfolios.length, [
                'работа',
                'работы',
                'работ',
              ])}`
            : 'Пока не добавлено работ'}
        </p>
      </div>
