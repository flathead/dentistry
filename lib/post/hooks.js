...
export function useSpecPages({ limit = 10 } = {}) {
  const { data, error, size, ...props } = useSWRInfinite(
    (index, previousPageData) => {
      // reached the end
      if (previousPageData && previousPageData.specialists.length === 0)
        return null;
      ...
      if (index !== 0) {
        ...
        const before = new Date(
          new Date(
            previousPageData.specialists[
              previousPageData.specialists.length - 1
            ].createdAt
          ).getTime()
        );
  ...
  const isEmpty = data?.[0]?.length === 0;
  const isReachingEnd =
    isEmpty || (data && data[data.length - 1]?.specialists?.length < limit);
