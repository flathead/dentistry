...
export function useNewsPages({ creatorId, limit = 100 } = {}) {
  const { data, error, size, ...props } = useSWRInfinite(
    (index, previousPageData) => {
      // reached the end
      if (previousPageData && previousPageData.news.length === 0) return null;
      ...
      if (index !== 0) {
        ...
        const before = new Date(
          new Date(
            previousPageData.news[previousPageData.news.length - 1].createdAt
          ).getTime()
        );
  ...
  const isReachingEnd =
    isEmpty || (data && data[data.length - 1]?.news?.length < limit);
