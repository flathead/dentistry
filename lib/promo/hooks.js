...
export function usePromoPages({ creatorId, limit = 10 } = {}) {
  const { data, error, size, ...props } = useSWRInfinite(
    (index, previousPageData) => {
      // reached the end
      if (previousPageData && previousPageData.promos.length === 0) return null;
      ...
      if (index !== 0) {
        ...
        const before = new Date(
          new Date(
            previousPageData.promos[
              previousPageData.promos.length - 1
            ].createdAt
          ).getTime()
        );
  ...
  const isReachingEnd =
    isEmpty || (data && data[data.length - 1]?.promos?.length < limit);
